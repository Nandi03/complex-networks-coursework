...
# Load categories from centrality DataFrame 
...
def categorise_positions(G, categories):
    ...
    # Initialise positions dictionary
    positions = {}
...
# Use existing graph and categories
positions = categorise_positions(G, categories)

# Plot the network
plt.figure(figsize=(12, 8))
colours = {"Core": "red", "Semi-Periphery": "green", "Periphery": "blue"}
node_colours = [colours[categories[node]] for node in G.nodes]

nx.draw(
    G, pos=positions, with_labels=True, node_size=500,
    node_color=node_colours, edge_color="gray", font_size=8
)
